...
const state = { pdfDoc:null, pageNum:1, pageCount:0, viewport:null, mode:null, scalePoints:[], boundary:[], existingPoints:[], proposedPoints:[], feetPerPixel:null, viewZoom:1 };
const el = Object.fromEntries(['pdfFile','pdfCanvas','overlayCanvas','viewer','pageInfo','prevPage','nextPage','modeStatus','cursorStatus','knownDistance','saveScale','scaleStatus','modeScale','modeBoundary','clearBoundary','pointElevation','modeExisting','modeProposed','undoPoint','clearPoints','gridSpacing','adjustPct','runCalc','results','loadStatus','pointCounts','zoomIn','zoomOut','zoomFit','zoomStatus'].map(id=>[id,document.getElementById(id)]));
...
function updateZoomStatus(){ if(el.zoomStatus) el.zoomStatus.textContent = `${Math.round(state.viewZoom*100)}%`; }
function applyDisplayZoom(){ if(!state.viewport) return; const w = Math.ceil(state.viewport.width * state.viewZoom); const h = Math.ceil(state.viewport.height * state.viewZoom); [el.pdfCanvas, el.overlayCanvas].forEach(c=>{ c.style.width = `${w}px`; c.style.height = `${h}px`; }); updateZoomStatus(); }
function setZoom(newZoom){ state.viewZoom = Math.max(0.5, Math.min(4, newZoom)); applyDisplayZoom(); }
function fitToWidth(){ if(!state.viewport) return; const available = Math.max(200, el.viewer.clientWidth - 8); state.viewZoom = available / state.viewport.width; applyDisplayZoom(); }
...
function getCanvasPoint(evt){ const rect = el.overlayCanvas.getBoundingClientRect(); const touch = evt.touches?.[0] || evt.changedTouches?.[0]; const clientX = touch ? touch.clientX : evt.clientX; const clientY = touch ? touch.clientY : evt.clientY; const scaleX = el.overlayCanvas.width / rect.width; const scaleY = el.overlayCanvas.height / rect.height; return { x: (clientX - rect.left) * scaleX, y: (clientY - rect.top) * scaleY }; }
...
async function renderPage(){ try{ const page = await state.pdfDoc.getPage(state.pageNum); const unscaled = page.getViewport({scale:1}); const targetWidth = Math.max(900, el.viewer.clientWidth - 20 || 900); const scale = targetWidth / unscaled.width; const viewport = page.getViewport({scale}); state.viewport = viewport; [el.pdfCanvas, el.overlayCanvas].forEach(c=>{ c.width = Math.ceil(viewport.width); c.height = Math.ceil(viewport.height); }); el.viewer.style.minHeight = `${Math.max(520, Math.min(viewport.height + 30, 1400))}px`; el.viewer.scrollTop = 0; el.viewer.scrollLeft = 0; await page.render({canvasContext:pdfCtx, viewport}).promise; el.pageInfo.textContent = `Page ${state.pageNum} / ${state.pageCount}`; fitToWidth(); drawOverlay(); } catch(err){ console.error(err); setStatus(`Page failed to render: ${err?.message || err}`, true); } }
...
el.zoomIn?.addEventListener('click', ()=>setZoom(state.viewZoom * 1.25));
el.zoomOut?.addEventListener('click', ()=>setZoom(state.viewZoom / 1.25));
el.zoomFit?.addEventListener('click', ()=>fitToWidth());
updateZoomStatus();